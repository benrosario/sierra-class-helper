...
Importing core is side-effect free and none of these tests need a SearchIndex,
an OpenAI key, or a built FAISS index — they exercise the pure module-level
helpers directly with in-memory data.
"""
from src.embeddings import core

# Deterministic subject whitelist for exact-code resolution tests.
_VALID = {"MATH", "PHYS", "CHEM", "CSCI", "BIOL", "BUS", "ENGL"}
...
class TestResolveCourseCode:
    def test_subject_word_plus_number(self):
        # The headline fix: the *word* "physics" + a number resolves to PHYS0205.
        assert core.resolve_course_code("physics 205", _VALID) == ("PHYS", "0205")

    def test_real_code_still_works(self):
        assert core.resolve_course_code("MATH 31", _VALID) == ("MATH", "0031")

    def test_abbreviated_code(self):
        assert core.resolve_course_code("phys 205", _VALID) == ("PHYS", "0205")

    def test_no_number_is_not_a_code(self):
        assert core.resolve_course_code("easy math class", _VALID) == (None, None)

    def test_non_adjacent_number_is_not_a_code(self):
        # "biology" and "20" aren't adjacent, so this is a topic search, not BIOL0020.
        assert core.resolve_course_code("20 best biology classes", _VALID) == (None, None)


class TestLexicalSearch:
    def test_linear_algebra_beats_college_algebra(self):
        entries = [
    ...
    def test_number_token_matches_padded_and_bare(self):
    ...
    def test_year_does_not_prefix_match_course_number(self):
...
class TestAllSectionsFor:
    def test_expands_to_all_sections(self):
        entries = [
    ...
    def test_preserves_course_order_and_dedups_input(self):
        entries = [
    ...
    def test_top_course_full_others_capped(self):
...
class TestFuseCandidates:
    def test_strong_lexical_beats_top_vector(self):
    ...
    def test_dedupes_by_course_code(self):
    ...
    def test_skips_already_seen_codes(self):
    ...
    def test_preferred_subject_boost(self):
